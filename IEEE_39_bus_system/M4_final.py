...
def preprocess(mpc, data_dir, all_data=False):
    ...
    for i in range(sample_num):
        D = np.diag((-input_B_diag[i, :, 0]) ** (-0.5))
    ...
    s2 = 29000
    if all_data:
        return [input_e, input_f, inputs_, k1, k2, input_G_ndiag, input_B_ndiag, input_G_diag, input_B_diag,
...
if __name__ == '__main__':
    ...
    # testing_scale = [100, 500, 700]
    #
    # testing_errors = []
    # for scale in testing_scale:
    #     saved_folder = "./models/M4_{}".format(scale)
    ...
    #     train_data[2] = torch.tensor(train_data[2], dtype=torch.float32).to(device)
    #     train_data[3] = torch.tensor(train_data[3], dtype=torch.float32).to(device)
    #     train_data[4] = torch.tensor(train_data[4], dtype=torch.float32).to(device)
    #     train_data[5] = torch.tensor(train_data[5], dtype=torch.float32).to(device)
    #     train_data[6] = torch.tensor(train_data[6], dtype=torch.float32).to(device)
    #     train_data[7] = torch.tensor(train_data[7], dtype=torch.float32).to(device)
    #     train_data[8] = torch.tensor(train_data[8], dtype=torch.float32).to(device)
    #     train_data[9] = torch.tensor(train_data[9], dtype=torch.float32).to(device)
    #     train_data[10] = torch.tensor(train_data[10], dtype=torch.float32).to(device)
    #     train_data[11] = torch.tensor(train_data[11][:, :, np.newaxis], dtype=torch.float32).to(device)
    #
    #     # 进行训练
    #     epoch_num = 1000
    #     losses_train = []
    ...
    #
    #         # 训练完成之后使用测试集进行测试
    #         res = model(test_data[0], test_data[1], test_data[2], test_data[3], test_data[4], test_data[5],
    #                     test_data[6],
    #                     test_data[7], test_data[8], test_data[9], test_data[10])
    #
    #         res = res.detach().cpu().numpy()[:, :, 0] * input_Pd_original_testing
    #         label = test_data[11].detach().cpu().numpy() * input_Pd_original_testing
    ...
    #         print(
    #             "Scale:{}, Epoch {:03d}: Train Loss: {:.6f}, Test Loss: {:.6f}".format(scale, epoch,
    #                                                                                    np.mean(epoch_loss),
    #                                                                                    np.mean(error)))
    #
    ...
    #
    #             save_csv(np.array([[best_model]]), "./M4_test_mae.csv")
    #
    #     print('finish!')

    # 计算可靠性指标

    eval_data[0] = torch.tensor(eval_data[0], dtype=torch.float32).to(device)
    eval_data[1] = torch.tensor(eval_data[1], dtype=torch.float32).to(device)
    eval_data[2] = torch.tensor(eval_data[2], dtype=torch.float32).to(device)
    eval_data[3] = torch.tensor(eval_data[3], dtype=torch.float32).to(device)
    eval_data[4] = torch.tensor(eval_data[4], dtype=torch.float32).to(device)
    eval_data[5] = torch.tensor(eval_data[5], dtype=torch.float32).to(device)
    eval_data[6] = torch.tensor(eval_data[6], dtype=torch.float32).to(device)
    eval_data[7] = torch.tensor(eval_data[7], dtype=torch.float32).to(device)
    eval_data[8] = torch.tensor(eval_data[8], dtype=torch.float32).to(device)
    eval_data[9] = torch.tensor(eval_data[9], dtype=torch.float32).to(device)
    eval_data[10] = torch.tensor(eval_data[10], dtype=torch.float32).to(device)
    eval_data[11] = torch.tensor(eval_data[11], dtype=torch.float32).to(device)

    model.load_state_dict(torch.load("./models/M4_19000/Model.pth"))
    reses = []
    batch_size = 1000
    for i in range(int(np.ceil(eval_data[0].shape[0] / batch_size))):
        ...
        res = model(eval_data[0][from_idx:to_idx, :, :],
                    ...
                    eval_data[8][from_idx:to_idx, :, :],
                    eval_data[9][from_idx:to_idx, :, :],
                    eval_data[10][from_idx:to_idx, :, :]
                    )
    ...
    output_cut_eval = eval_data[11].detach().cpu().numpy()[:, :, np.newaxis] * input_Pd_original_eval
