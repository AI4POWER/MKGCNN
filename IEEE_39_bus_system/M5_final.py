...
def preprocess(mpc, data_dir, all_data=False):
    ...
    if all_data:
        return [input_e, input_f, inputs_, k1, k2, input_G_ndiag, input_B_ndiag, input_G_diag, input_B_diag,
            input_Pd, input_Qd, output_cut], input_Pd_orginal[:, :, 0]
    else:
...
if __name__ == '__main__':
    ...
    test_data, input_Pd_original_testing = eval_data, input_Pd_original_eval
    ...
    output_cut_eval = eval_data[11].detach().cpu().numpy() * input_Pd_original_eval
    ...
    #         print(
    #             "Scale:{}, Epoch {:03d}: Train Loss: {:.6f}, Test Loss: {:.6f}, lolp:{:.6f}, e_lolp:{:.6f}, eens:{:.6f}, e_eens:{:.6f}:".format(scale, epoch,
    #                                                                                    np.mean(epoch_loss),
    #                                                                                     np.mean(error),
    ...
    # eval_data, input_Pd_original_eval = test_data, input_Pd_original_testing
    ...
    model.load_state_dict(torch.load("./models/M5_19000/Model.pth"))
    reses = []
    batch_size = 1000
    import time
    t1 = time.time()
