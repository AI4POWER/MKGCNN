...
class GCNLayer(nn.Module):
    ...
    def forward(self, ef, input_k1):
        ...
        ef_new = torch.tanh(self.fc_v1(ef1))
...
def preprocess(mpc, data_dir):
    ...
    for i in range(sample_num):
        k = 0.01
        # k = 0.001
        A = np.sqrt(np.square(input_G_ndiag[i, :, :]) + np.square(input_B_ndiag[i, :, :]))
        A = np.exp(-k * A)
        A[A == 1.0] = 0
        A = np.array(A)
        k1[i, :, :] = A
...
if __name__ == '__main__':
    ...
    inputs_eval, output_cut_eval, k1_eval, input_Pd_original_eval = preprocess(mpc,
                                                                               "./dataset/case_39_evaluation_testing_samples[30000].npz")
    ...
    input_Pd_original_training, input_Pd_original_testing = input_Pd_original[:20000, :, 0], input_Pd_original[
                                                                                             20000:30000, :, 0]
    ...
    for epoch in range(epoch_num):  # loop over the dataset multiple times
        epoch_loss = []
        for fs, labels in data_generator(train_data, batch_size=200):
            optimizer.zero_grad()
        ...
        if epoch % 10 == 0:
            #训练完成后更新保存模型
            model_path = saved_folder + "/M2_Model.pth"
    ...
    # model.load_state_dict(torch.load("./models/M2_new2/M2_Model.pth"))
    # res = model(test_data[0], test_data[1])
    # res = res.detach().cpu().numpy()
    # label = test_data[2].detach().cpu().numpy()
    # error = np.abs(res - label)
    # # np.savez('./results/M2_results.npz', error=error)
    #
    #
    ...
    # res = res.detach().cpu().numpy()
    # res[res < 0.0001] = 0
    # output_cut = res * input_Pd_original_eval[:, :, 0]
    ...
    #
    # output_cut_eval = output_cut_eval.detach().cpu().numpy()*input_Pd_original_eval
