...
def preprocess(mpc, data_dir, all_data=False):
    ...
    if all_data:
        ...
        input_pq_Bdiag[20000:, :, :], output_cut[20000:, :]], input_Pd_orginal[:20000, :, 0], input_Pd_orginal[20000:, :, 0]
...
if __name__ == '__main__':
    ...
    # 导入训练数据
    data_dir = "./dataset/case_39_evaluation_training_samples[30000].npz"
    ...
    input_Pd = data['input_Pd'] / 100
    input_Qd = data['input_Qd'] / 100
    ...
    input_G_diag = np.array([np.diag(input_G[i, :, :]) for i in range(input_Pd.shape[0])])
    (input_G, input_B, input_G_diag, input_B_diag, input_Pd, input_Qd,
     input_Gen, output_cut) = (input_G[idx, :, :], input_B[idx, :, :],
                               input_G_diag[idx, :], input_B_diag[idx, :], input_Pd[idx, :], input_Qd[idx, :], input_Gen[idx, :], output_cut[idx, :])

    import time
    t1 = time.time()
    # 构建输入特征向量
    input_pq_Bdiag = np.concatenate([input_Pd[:, :, np.newaxis], input_Qd[:, :, np.newaxis],
                                     input_B_diag[:, :, np.newaxis], input_G_diag[:, :, np.newaxis]], axis=-1)[:, np.newaxis, :, :]
    ...
    output_cut_ = res * input_Pd
    p = np.mean(output_cut_, axis=1)
    idx = np.where(p > 0.01)[0]
    lolp1 = idx.shape[0] / p.shape[0]
    EENS1 = np.sum(output_cut_) * 100 / input_Pd.shape[0] * 8760

    t2 = time.time()


    output_cut_eval = output_cut
    p = np.mean(output_cut_eval, axis=1)
    idx = np.where(p > 0.001)[0]
    lolp0 = idx.shape[0] / p.shape[0]
    EENS0 = np.sum(output_cut_eval) * 100 / input_Pd.shape[0] * 8760
