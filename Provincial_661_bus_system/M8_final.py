...
if __name__ == '__main__':
    ...
    inputs = np.concatenate([input_Pd, input_Qd, input_Gen, input_Br], axis=-1)
    output_cut = np.sum(output_cut, axis=-1)
    ...
    input_Gen = data['input_Gen']
    input_Br = data['input_Br']
    output_cut_initial = copy.deepcopy(output_cut)
    ...
    # 计算基准值
    output_cut_eval = output_cut_initial
    ...
    #     # res[res<0] = 0
    #     # res[res>1] = 1
    #     output_cut = res.detach().cpu().numpy()
    #     output_cut[output_cut<0.001] = 0

    #     p = np.sum(output_cut, axis=1)
    #     idx = np.where(p > 0)[0]
    #     lolp1 = idx.shape[0] / p.shape[0]
    #     EENS1 = np.sum(output_cut) * 100 / train_data[0].shape[0] * 8760

    #     e_lolp = abs(lolp1 - lolp0) / lolp0
    #     e_eens = abs(EENS1 - EENS0) / EENS0

    #     res = res.detach().cpu().numpy()
    #     label = train_data[1].detach().cpu().numpy()
    #     error = np.mean(np.abs(res - label))
    ...
    output_cut = res.detach().cpu().numpy()
    # output_cut[output_cut<0.001] = 0
    idx = np.where(output_cut > 0.5)[0]
    lolp1 = idx.shape[0] / output_cut.shape[0]
    EENS1 = np.sum(output_cut_initial[idx, :]) * 100 / test_data[0].shape[0] * 8760
