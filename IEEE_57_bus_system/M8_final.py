...
def preprocess(mpc, data_dir, all_data=False):
    ...
    # 对切负荷量进行归一化处理
    output_cut_orginal = copy.deepcopy(output_cut)
    ...
    # 构建机组与节点的关联矩阵
    input_Pg = np.zeros((sample_num, 7, 1))
    input_Qg = np.zeros((sample_num, 7, 1))
    ...
    if all_data:
        return [input_pq_Bdiag, output_cut[:, :]], output_cut_orginal[:, :, 0]
    else:
        return [input_pq_Bdiag[:20000, :], output_cut[:20000, :]], [
        input_pq_Bdiag[30000:60000, :], output_cut[30000:60000, :]], output_cut_orginal[:20000, :,
                                                                    0], output_cut_orginal[30000:60000, :, 0]
...
if __name__ == '__main__':
    ...
    train_data, test_data, output_cut_original_training, output_cut_original_testing = preprocess(mpc, data_dir)
    # except:
    #     data_dir = "./dataset/case_57_training_sample_dist[1.0_0.5]_sample[60000].npz"
    #     train_data, test_data, output_cut_original_training, output_cut_original_testing = preprocess(mpc, data_dir)

    # 计算基准值
    output_cut_eval = output_cut_original_testing
    output_cut_eval[output_cut_eval < 0.001] = 0
    p = np.sum(output_cut_eval, axis=1)
    idx = np.where(p > 0)[0]
    lolp0 = idx.shape[0] / p.shape[0]
    EENS0 = np.sum(output_cut_eval) * 100 / output_cut_original_testing.shape[0] * 8760
    print(lolp0, EENS0)
    ...
    #
    #     p = res.detach().cpu().numpy()
    #     idx = np.where(p > 0.5)[0]
    #     lolp1 = idx.shape[0] / p.shape[0]
    #     EENS1 = np.sum(output_cut_original_testing[idx, :]) * 100 / output_cut_original_testing.shape[0] * 8760
    #
    #     e_lolp = abs(lolp1 - lolp0) / lolp0
    #     e_eens = abs(EENS1 - EENS0) / EENS0
    #
    #     res = res.detach().cpu().numpy()
    #     error = np.abs(np.sum(res) - np.sum(test_data[1].detach().cpu().numpy()))
    #
    ...
    t2 = time.time()
    opt_time = idx.shape[0]* 0.1562206# 平均一个样本的计算时间为0.1562206s

    e_lolp = abs(lolp1 - lolp0) / lolp0
    e_eens = abs(EENS1 - EENS0) / EENS0
    print(lolp1, e_lolp, EENS1, e_eens)
    print(lolp0, EENS0)
    print(t2-t1 + opt_time)
