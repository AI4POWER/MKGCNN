...
class GCNLayer(nn.Module):
    ...
    def forward(self, e, f, k1, k2, G_ndiag, B_ndiag, G_diag, B_diag, Pd, Qd):
        ...
        base_gb = G_diag * G_diag + B_diag * B_diag + 1
        ...
        base4 = G_diag * G_diag + B_diag * B_diag + 1
...
def preprocess(mpc, data_dir):
    ...
    for i in range(sample_num):
        k = 0.01
        A = np.sqrt(np.square(input_G[i].todense()) + np.square(input_B[i].todense()))
        A = np.exp(-k * A)
        A[A == 1.0] = 0
        A = csr_matrix(np.array(A))
        k1.append(A)
    k1 = np.array(k1)
    ...
    for i in range(sample_num):
        ...
        kernel = csr_matrix(np.dot(np.dot(D, (input_B_ndiag[i].todense() + In)), D))
        k2.append(kernel)
    k2 = np.array(k2)
    ...
    s1 = 30000
    s2 = 60000
    return (input_e[:s0, :, :], input_f[:s0, :, :], inputs[:s0, :, :], k1[:s0], k2[:s0],
            input_G_ndiag[:s0], input_B_ndiag[:s0],
            input_G_diag[:s0], input_B_diag[:s0, :, :],
            input_Pd[:s0, :, :], input_Qd[:s0, :, :], output_cut[:s0, :]), \
        [input_e[s1:s2, :, :], input_f[s1:s2, :, :], inputs[s1:s2, :, :], k1[s1:s2], k2[s1:s2],
...
def data_generator(train_data, input_Pd_original_training, batch_size=15):
    ...
    for k in range(int(np.ceil(e.shape[0] / batch_size))):
        ...
        k1_, k2_, G_ndiag_, B_ndiag_ = [], [], [], []
        for i in range(index.shape[0]):
            k1_.append(k1[i].todense())
            k2_.append(k2[i].todense())
            G_ndiag_.append(G_ndiag[i].todense())
            B_ndiag_.append(B_ndiag[i].todense())
...
if __name__ == '__main__':
    ...
    output_cut_eval = test_data[11] * input_Pd_original_testing
    ...
    for scale in testing_scale:
        ...
        for epoch in range(epoch_num):  # loop over the dataset multiple times
            ...
            reses = []
            pds = []
            for fs, labels, pd_ in data_generator(test_data, input_Pd_original_testing, batch_size=100):

                fs[0] = torch.tensor(fs[0], dtype=torch.float32).to(device)
                fs[1] = torch.tensor(fs[1], dtype=torch.float32).to(device)
                fs[2] = torch.tensor(fs[2], dtype=torch.float32).to(device)
                fs[3] = torch.tensor(fs[3], dtype=torch.float32).to(device)
                fs[4] = torch.tensor(fs[4], dtype=torch.float32).to(device)
                fs[5] = torch.tensor(fs[5], dtype=torch.float32).to(device)
                fs[6] = torch.tensor(fs[6], dtype=torch.float32).to(device)
                fs[7] = torch.tensor(fs[7], dtype=torch.float32).to(device)
                fs[8] = torch.tensor(fs[8], dtype=torch.float32).to(device)
                fs[9] = torch.tensor(fs[9], dtype=torch.float32).to(device)
                fs[10] = torch.tensor(fs[10], dtype=torch.float32).to(device)
                labels = torch.tensor(labels, dtype=torch.float32).to(device)

                res = model(fs[0], fs[1], fs[2], fs[3], fs[4], fs[5], fs[6], fs[7], fs[8], fs[9], fs[10])
            ...
            res = res * pds
            label = test_data[11] * pds
            error = np.abs(np.sum(res) - np.sum(label))
            error2 = np.mean(np.abs(res - label))
            ...
            print(
                "Scale:{}, Epoch {:03d}: Train Loss: {:.6f}, Test Loss: {:.6f}, {:.6f}, lolp:{:.6f}, e_lolp:{:.6f}, eens:{:.6f}, e_eens:{:.6f}:".format(
                    scale, epoch,
                    np.mean(epoch_loss),
                    np.mean(error), error2,
                    lolp1, e_lolp, EENS1, e_eens))

            if np.mean(error2) < best_model:
                best_model = np.mean(error2)
                # 训练完成后更新保存模型
                model_path = saved_folder + "/Model.pth"
                torch.save(model.state_dict(), model_path)

                save_csv(np.array([[best_model]]), "./M5_test_mae_sparse.csv")
