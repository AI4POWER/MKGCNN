...
def Monte_Carlo(ppc):
    ...
    PV = ppc['PV']
    # # 对负荷抽样，假定负荷均服从正态分布
    load_e_p = np.random.normal(loc=bus[:, PD]*1, scale=np.abs(bus[:, PD]) * 0.3)
    load_e_q = np.random.normal(loc=bus[:, QD]*1, scale=np.abs(bus[:, QD]) * 0.3)

    load_e_p[load_e_p < 0] = 0
    # print(load_e_p)
...
def sampling(mpc, index):
    ...
    branch[:, BR_STATUS] = 1
    gen[:, GEN_STATUS] = 1
    temp = index % (branch.shape[0] + gen.shape[0] + 20)
    if temp < branch.shape[0]:
        branch[temp, BR_STATUS] = 0
    elif temp >= branch.shape[0] and temp < branch.shape[0] + gen.shape[0]:
        gen[temp - branch.shape[0], GEN_STATUS] = 0
        gen[1, GEN_STATUS] = 1  # 平衡机不发生故障
    ...
    branch[13, BR_STATUS] = 1  # 第14条线路的故障率设置为0，避免系统解裂
...
def task(mpc):
    ...
    while i < sample_number:
        index += 1
        # 根据负荷分布和故障率进行抽样
        sampled_mpc = sampling(copy.deepcopy(mpc), index)
        print(i, cut_s)
        ...
        if status:
            ...
            input_G.append(np.real(Ybus.todense()))
            input_B.append(np.imag(Ybus.todense()))
...
if __name__ == '__main__':
    ...
    mpc['windF'] = np.array([[0, 20, 2.016, 5.089, 3.5, 15, 25],
                             [2, 20, 2.016, 5.089, 3.5, 15, 25],
                             ])
    mpc['PV'] = np.array([[7, 20, 2.06, 2.5],
                          [8, 20, 2.06, 2.5],
                          ])
    # 计算新能源的渗透率
    ...
    np.savez('./dataset/case_118_training_dist[1_0.3]_sample[{}].npz'.format(input_Pd.shape[0]),
